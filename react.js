...
class App extends React.Component {
    ...
    handleMarked(name){
        const cars = this.state.cars.concat() // local copy cars!!! локальная копия массива

        // исключает возможность подсвечивания на более чем 1 карточке
        const mar = cars.find(m => m.marked === true && m.name !== name )
        //console.log(mar)
        if(mar !== undefined)
        mar.marked = !mar.marked

        // Подсветка и ее отключение для 1 карты
        const car = cars.find(c => c.name === name)
        car.marked = !car.marked

        // this.setState({cars: cars})  //{array: local copy}
        this.setState({cars})  // cars == cars => {cars}
    }
        ...
        return (
            <div className="app">
                ...
                <input
                    ...
                    onChange={(event) => this.titleChangeHandler(event.target.value)} //listener changes
                />
